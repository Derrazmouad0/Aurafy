...
function SeasonSectionContent({ tvId, seasons, fallbackPoster }: { tvId: string, seasons: any[], fallbackPoster: string }) {
  ...
  const handleSeasonClick = async (seasonNumber: number) => {
    if (activeSeason === seasonNumber) { setActiveSeason(null); return; }
    ...
    <section>
      ...
      <div className="flex gap-4 overflow-x-auto py-6 px-2 -mx-2 scrollbar-hide">
        {seasons.filter((s: any) => s.season_number > 0).map((season: any) => (
          <div key={season.id} onClick={() => handleSeasonClick(season.season_number)} className={`flex-shrink-0 w-40 cursor-pointer group transition-all duration-300 ${activeSeason === season.season_number ? 'scale-105' : 'hover:scale-105 opacity-70 hover:opacity-100'}`}>
            <div className={`aspect-[2/3] relative rounded-xl overflow-hidden border-2 shadow-lg mb-3 ${activeSeason === season.season_number ? 'border-accentPink glow-pink' : 'border-white/5 group-hover:border-white/20'}`}>
              <Image src={season.poster_path ? `https://image.tmdb.org/t/p/w200${season.poster_path}` : `https://image.tmdb.org/t/p/w200${fallbackPoster}`} alt={season.name} fill className="object-cover" />
            </div>
          ...
          {isLoading ? (
            ...
            <div className="space-y-6">
              <div className="flex justify-between items-start">
                ...
                <button onClick={() => setActiveSeason(null)} className="text-textGray hover:text-white bg-white/5 p-2 rounded-full">✕</button>
              </div>
              <p className="text-textGray font-light leading-relaxed">{seasonData.overview || (isEnglish ? "No synopsis available for this season." : "Le synopsis de cette saison n'est pas encore disponible.")}</p>
              <div className="pt-4 border-t border-white/5">
                <h4 className="text-sm font-bold tracking-widest text-textGray uppercase mb-4">{isEnglish ? "Episode List" : "Liste des épisodes"}</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-96 overflow-y-auto pr-2 custom-scrollbar">
                  {seasonData.episodes?.map((ep: any) => (
                    <div key={ep.id} onClick={() => handleEpisodePlay(ep.episode_number)} className="flex gap-4 p-3 bg-deepBlack/50 rounded-xl border border-white/5 hover:border-accentPink hover:bg-accentPink/10 transition-colors cursor-pointer group">
      ...
      {playerOpen && activeSeason && selectedEpisode && (
        <VideoPlayerModal mediaType="tv" mediaId={tvId} season={activeSeason} episode={selectedEpisode} onClose={() => setPlayerOpen(false)} />
      )}
    </section>
  );
}

export default function SeasonSection(props: { tvId: string, seasons: any[], fallbackPoster: string }) {
  return (
    <Suspense fallback={null}>
      <SeasonSectionContent {...props} />
    </Suspense>
  );
}